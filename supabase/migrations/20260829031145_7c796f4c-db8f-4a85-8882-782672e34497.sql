...
CREATE POLICY "Public can read visible faqs" ON public.faqs
  FOR SELECT TO anon, authenticated USING (is_visible = true OR auth.uid() IS NOT NULL);
...
CREATE POLICY "Public can read approved testimonials" ON public.testimonials
  FOR SELECT TO anon, authenticated USING (is_approved = true OR auth.uid() IS NOT NULL);
...
GRANT SELECT ON public.menu_packages TO anon;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.menu_packages TO authenticated;
...
CREATE POLICY "Public can read enabled menus" ON public.menu_packages
  FOR SELECT TO anon, authenticated USING (is_enabled = true OR auth.uid() IS NOT NULL);
